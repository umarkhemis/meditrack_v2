...
def get_checkin_message(patient):
    ...
    return (
        f"Hello {first_name}, your care team is checking in on your recovery.\n\n"
        f"Reply with the numbers that apply (comma-separated):\n"
        f"1 - Fever\n"
        f"2 - Pain\n"
        f"3 - Breathing difficulty\n"
        f"4 - Wound issues\n"
        f"5 - Nausea/Vomiting\n"
        f"6 - Dizziness\n"
        f"0 - Feeling well\n\n"
        f"Example: reply 1,3 if you have fever and breathing difficulty.\n"
...
def handle_inbound_sms(from_number, body):
    ...
    Patients reply with comma-separated numbers corresponding to symptoms:
        0 - Feeling well
        1 - Fever
        2 - Pain
    ...
    Example reply: "1,3" → fever + breathing difficulty → status red
    The parsed reply creates a CheckIn record and triggers alerts if needed.
    """
    from .models import Patient, SMSLog, CheckIn
    ...
    SMSLog.objects.create(
        ...
        status='received', channel=channel,
    )
    logger.info(f"Inbound from {patient.full_name}: {body[:60]}")

    # Parse the comma-separated numbers
    # Strip whitespace, handle "STOP" opt-out
    body_clean = body.strip().upper()
    if body_clean == 'STOP':
        reply = f"You have been unsubscribed, {first_name}. Contact your care team if this was a mistake."
        return (
            '<?xml version="1.0" encoding="UTF-8"?>'
            f'<Response><Message>{reply}</Message></Response>'
        )

    # Extract digits from reply like "1,3,5" or "1, 3, 5" or "1 3 5"
    import re
    digits = re.findall(r'\d', body_clean)

    if not digits:
        # Not a symptom reply — log and respond helpfully
        reply = (
            f"Hi {first_name}, to report your symptoms reply with numbers separated by commas.\n"
            f"0=Feeling well, 1=Fever, 2=Pain, 3=Breathing difficulty, "
            f"4=Wound issues, 5=Nausea/Vomiting, 6=Dizziness.\n"
            f"Example: 1,3"
        )
        return (
            '<?xml version="1.0" encoding="UTF-8"?>'
    ...
    # Map digit codes to CheckIn fields
    code_map = {
        '0': 'feeling_well',
        '1': 'fever',
        '2': 'pain',
        '3': 'difficulty_breathing',
        '4': 'wound_issues',
        '5': 'nausea_vomiting',
        '6': 'dizziness',
    }
    ...
    checkin_data = {
        ...
        'dizziness': False,
        'feeling_well': False,
        'additional_notes': f'Reported via SMS: "{body.strip()}"',
    }

    valid_codes = []
    for d in digits:
        field = code_map.get(d)
        if field:
            checkin_data[field] = True
            valid_codes.append(d)

    # Create the CheckIn — status is auto-calculated in model save()
    checkin = CheckIn.objects.create(**checkin_data)
    is_critical = checkin.status in ('yellow', 'red')

    if is_critical:
        create_doctor_alert(patient, checkin)

    # Build confirmation reply
    symptom_labels = {
        '0': 'Feeling well', '1': 'Fever', '2': 'Pain',
        '3': 'Breathing difficulty', '4': 'Wound issues',
        '5': 'Nausea/Vomiting', '6': 'Dizziness',
    }
    reported = ', '.join(symptom_labels[d] for d in valid_codes if d in symptom_labels)

    if checkin.status == 'red':
        reply = (
            f"Thank you {first_name}. We have recorded: {reported}. "
            f"Your doctor has been urgently notified. "
            f"If your condition is worsening now, please go to the nearest hospital or call emergency services."
        )
    elif checkin.status == 'yellow':
        reply = (
            f"Thank you {first_name}. We have recorded: {reported}. "
            f"Your care team has been notified and will follow up with you soon."
        )
    else:
        reply = (
            f"Thank you {first_name}. We have recorded: {reported}. "
            f"Keep following your care plan and stay well."
        )

    logger.info(
        f"CheckIn created via SMS for {patient.full_name} — "
        f"status: {checkin.status}, codes: {valid_codes}"
    )
